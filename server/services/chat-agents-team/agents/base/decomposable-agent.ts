...
export abstract class DecomposableAgent extends BaseAgent {
  async execute(options: AgentExecuteOptions): Promise<AgentExecution> {
    ...
    if (this.definition.selfDecompose) {
      ...
      const leafTasks = await this.flattenToLeafTasks(
        options.prompt,
        options,
        0,
      );
  ...
  private async flattenToLeafTasks(
    ...
    for (const sub of assessment.subtasks) {
      const childLeaves = await this.flattenToLeafTasks(
        sub.prompt,
        options,
        depth + 1,
      );
  ...
  private async executeSubTasks(
    ...
    for (let i = 0; i < subtasks.length; i++) {
      ...
      if (execution.status === "error") {
        ...
        this.emitStandalone({
          type: "activity",
          description: `Sub-task "${sub.title}" failed — stopping`,
        });
    ...
    this.emitStandalone({
      type: "activity",
      description: `All ${subtasks.length} sub-tasks completed`,
    });
