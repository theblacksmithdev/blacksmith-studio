...
import type { AiCompletionOptions, AiStreamOptions, AiStreamHandle, AiProviderStatus } from '../types.js'
...
export class ClaudeCliProvider extends AiProvider {
  ...
  getBin(): string {
    ...
    try {
      this.binPath = execSync('bash -ilc "which claude"', { encoding: 'utf-8', timeout: 5000 }).trim()
    } catch {
      this.binPath = 'claude'
    }
    return this.binPath
  ...
  complete(options: AiCompletionOptions): Promise<string | null> {
    const { prompt, systemPrompt, model = AiModelTier.Fast, cwd, timeout = 30000, disableTools } = options
    const bin = this.getBin()

    return new Promise((resolve) => {
      const args = ['--print', '--output-format', 'stream-json', '--model', this.resolveModel(model)]
      ...
      const timer = setTimeout(() => { proc.kill('SIGTERM'); resolve(null) }, timeout)
  ...
  stream(options: AiStreamOptions): AiStreamHandle {
    const {
      prompt, systemPrompt, model, cwd, nodePath,
      onChunk, sessionId, resume, permissionMode = 'bypassPermissions',
      mcpConfigPath, maxBudget, customInstructions, projectContext, disableTools,
    } = options
    const bin = this.getBin()
    ...
    const effectivePrompt = (!resume && projectContext)
    ...
    if (maxBudget != null && maxBudget > 0) args.push('--max-budget-usd', String(maxBudget))
    ...
    const promise = new Promise<void>((resolve, reject) => {
      ...
      proc.on('close', (code) => {
        ...
        else reject(new Error(stderrBuffer.trim() || `Process exited with code ${code}`))
      })
