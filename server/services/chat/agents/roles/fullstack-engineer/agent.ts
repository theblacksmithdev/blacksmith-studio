...
export class FullstackEngineerAgent extends DecomposableAgent {
  ...
  protected processResult(
    ...
    const pyFiles = toolCalls.filter(
      (tc) =>
        (tc.toolName === "Write" || tc.toolName === "Edit") &&
        typeof tc.input.file_path === "string" &&
        tc.input.file_path.endsWith(".py"),
    ).length;
    const tsFiles = toolCalls.filter(
      (tc) =>
        (tc.toolName === "Write" || tc.toolName === "Edit") &&
        typeof tc.input.file_path === "string" &&
        (tc.input.file_path.endsWith(".ts") ||
          tc.input.file_path.endsWith(".tsx")),
    ).length;
    const parts: string[] = [];
    if (filesCreated + filesEdited > 0) {
      parts.push(`${filesCreated + filesEdited} file(s) touched`);
      if (pyFiles > 0 && tsFiles > 0)
        parts.push(`(${pyFiles} backend, ${tsFiles} frontend)`);
    }
    if (parts.length === 0)
      return (
