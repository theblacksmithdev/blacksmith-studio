...
const IGNORE = new Set([
  ...
  'htmlcov', '.pytest_cache', '.mypy_cache', 'build', 'egg-info',
  'migrations', 'static', 'media', '.next', '.nuxt',
])

const KEY_FILES = [
  'package.json', 'pyproject.toml', 'requirements.txt',
  'blacksmith.config.json', 'CLAUDE.md', 'README.md',
  'tsconfig.json', 'vite.config.ts', 'next.config.js',
  'manage.py', 'settings.py', 'urls.py',
]

const CODE_EXTENSIONS = new Set([
  '.ts', '.tsx', '.js', '.jsx', '.py', '.css', '.html', '.json', '.yaml', '.yml',
])
...
function buildTree(
  root: string, dir: string, lines: string[], depth: number, maxDepth: number,
) {
  ...
  for (const entry of filtered) {
    ...
    } else if (CODE_EXTENSIONS.has(path.extname(entry.name)) || KEY_FILES.includes(entry.name)) {
      lines.push(`${indent}${entry.name}`)
    }
  }
}
