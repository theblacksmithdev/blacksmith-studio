...
const REPLAN_TRIGGERS = new Set<string>([
  "ui-designer",
  "architect",
  "database-engineer",
]);
...
export class AgentManager {
  ...
  async dispatch(
    ...
    if (routeResult.confidence === "high" && routeResult.role) {
      ...
      if (isExplicit) {
        ...
        const directTask: DispatchTask = {
          id: "t0",
          title: execution.prompt.slice(0, 60),
          description: "",
          role: routeResult.role,
    ...
    if (plan.mode === "single" && plan.task) {
      ...
      if (
        ...
        needsQualityGate(plan.task.role, plan.task.reviewLevel)
      ) {
    ...
    const executions = await this.executeTaskPlan(
      plan.tasks,
      options,
      artifacts,
    );
  ...
  private async runFinalQualityGate(
    context: {
      ...
      prompt: string;
      title: string;
      reviewLevel?: import("./pm-dispatcher.js").ReviewLevel;
    },
    options: AgentExecuteOptions,
  ...
  private async executeTaskPlan(
    ...
    for (let i = 0; i < tasks.length; i++) {
      ...
      try {
        ...
        const execution = await this.executeDispatchedTask(
          refinedTask,
          baseOptions,
          completed,
          artifactPaths,
        ...
        if (execution.status === "done") {
          ...
          if (execution.responseText.trim()) {
            try {
              ...
              console.warn(
                `[agent-manager] Failed to write artifact for ${task.id}:`,
                err.message,
              );
            }
          }
          ...
          if (
            ...
            try {
              ...
              console.warn(
                `[agent-manager] Re-plan failed, continuing with original tasks:`,
                err.message,
              );
            }
          }
          ...
          for (const remaining of tasks) {
            if (!completed.has(remaining.id)) {
              ...
              const skippedExec: AgentExecution = {
                ...
                costUsd: 0,
                durationMs: 0,
                error: `Skipped: "${task.title}" failed`,
                responseText: "",
              };
  ...
  private async executeDispatchedTask(
    ...
    const existingSession =
      pipelineSessions.get(task.role) ?? this.roleSessions.get(task.role);
