...
export async function executeWorkflowSteps(
  ...
  while (completed.size < steps.length) {
    // Budget guard
    if (workflow.maxBudgetUsd != null && workflow.totalCostUsd >= workflow.maxBudgetUsd) {
    ...
    await Promise.allSettled(
      ready.map((idx) => executeStep(workflow, idx, baseOptions, execute, emitWorkflow)),
    )
...
async function executeStep(
  ...
  emitWorkflow({
    ...
    data: { role: step.role, status: 'running', message: `Starting ${step.role}` },
  })

  try {
    const execution = await execute({ ...baseOptions, prompt, role: step.role })
    ...
    if (execution.status === 'done') {
      ...
      emitWorkflow({
        ...
        stepIndex,
        timestamp: new Date().toISOString(),
        data: { role: step.role, status: 'completed', costUsd: execution.costUsd, durationMs: execution.durationMs },
      })
    } else {
      step.status = 'failed'
