 ...
 */
export function computeChanges(projectRoot: string, snapshot: ChangeSnapshot): ChangeSet {
  ...
  try {
    ...
    const nameStatus = execSync(`git diff ${snapshot.commitHash} --name-status --no-color`, {
      cwd: projectRoot,
      encoding: 'utf-8',
      timeout: 10000,
    }).trim()
...
export function formatChangesForReview(changes: ChangeSet): string {
  ...
  if (changes.diff) {
    ...
    lines.push('## Diff\n```diff')
    lines.push(truncated ? changes.diff.slice(0, maxDiffSize) + '\n... (truncated)' : changes.diff)
    lines.push('```')
  }
