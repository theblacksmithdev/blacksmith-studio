...
type ProcessResultFn = (execution: AgentExecution, fullResponse: string, toolCalls: ToolCallRecord[]) => string | Promise<string>
type EvaluateHandoffFn = (fullResponse: string, toolCalls: ToolCallRecord[]) => HandoffDescriptor | null
...
export function streamExecution(opts: StreamOptions): Promise<AgentExecution> {
  const { proc, execution, emit, processResult, evaluateHandoff, getSettled, setSettled } = opts
  ...
  return new Promise((resolve) => {
    ...
    const finish = (status: AgentStatus, error?: string) => {
      if (getSettled()) { resolve(execution); return }
      setSettled(true)
    ...
    const parser = createNdjsonParser((chunk: any) => {
      ...
      handleChunk(chunk, execution, fullResponse, toolCalls, emit, evaluateHandoff, firstMessageEmitted, (text) => { fullResponse += text }, () => { firstMessageEmitted = true })
    })
    ...
    proc.on('error', (err) => {
      ...
      emit({ type: 'error', error: execution.error!, recoverable: false }, execution)
      resolve(execution)
    })
  ...
  if (chunk.type === 'assistant') {
    ...
    for (const block of contentBlocks) {
      if (block.type === 'text') {
        ...
        emit({ type: 'message', content: block.text, isPartial: !chunk.stop_reason }, execution)
        ...
        const record: ToolCallRecord = { toolId: block.id, toolName: block.name, input: block.input }
        toolCalls.push(record)
        emit({ type: 'tool_use', toolId: block.id, toolName: block.name, input: block.input }, execution)
    ...
    if (chunk.stop_reason) {
      ...
      if (handoff) {
        emit({ type: 'handoff', targetRole: handoff.targetRole, reason: handoff.reason, context: handoff.context }, execution)
      }
    }
