...
export class ProjectBuilder {
  ...
  async build(
    ...
    // ── 1. Plan ──
    this.emit({ type: 'build:plan_started', buildId, timestamp: now, data: { message: 'Analyzing requirements and generating build plan...' } })

    let plan: BuildPlan
    try {
      plan = await generatePlan(requirements, baseOptions)
    } catch (err: any) {
      this.emit({ type: 'build:failed', buildId, timestamp: new Date().toISOString(), data: { message: `Planning failed: ${err.message}` } })
      throw err
    }

    this.emit({ type: 'build:plan_ready', buildId, timestamp: new Date().toISOString(), data: { message: `Plan ready: ${plan.phases.length} phases, ${plan.totalTasks} tasks`, plan } })
    ...
    if (!approved) {
      this.emit({ type: 'build:cancelled', buildId, timestamp: new Date().toISOString(), data: { message: 'Build plan rejected by user' } })
  ...
  async resume(
    ...
    if (!checkpoint) throw new Error('No build checkpoint found. Start a new build instead.')
    ...
    for (const [id, result] of Object.entries(checkpoint.results)) {
      ...
      if (result.sessionId) this.roleSessions.set(result.role, result.sessionId)
    }
    ...
    this.activeBuild = progress
    this.emit({ type: 'build:phase_started', buildId: progress.id, phaseIndex: progress.currentPhase, timestamp: new Date().toISOString(), data: { message: `Resuming from phase ${progress.currentPhase + 1}/${progress.plan.phases.length}`, progress: this.progressSummary(progress, 'Resuming...') } })
  ...
  cancel(): void {
    ...
    this.emit({ type: 'build:cancelled', buildId: this.activeBuild.id, timestamp: this.activeBuild.completedAt, data: { message: 'Build cancelled by user' } })
  ...
  private async runBuild(
    ...
    try {
      ...
      const anyFailed = Array.from(progress.results.values()).some((r) => r.status === 'failed')
      progress.status = anyFailed ? 'failed' : 'completed'
      progress.completedAt = new Date().toISOString()

      if (progress.status === 'completed') deleteCheckpoint(baseOptions.projectRoot)

      this.emit({
        type: progress.status === 'completed' ? 'build:completed' : 'build:failed',
        buildId: progress.id,
      ...
      progress.completedAt = new Date().toISOString()
      this.emit({ type: 'build:failed', buildId: progress.id, timestamp: progress.completedAt, data: { message: err.message } })
    } finally {
      this.activeBuild = null
    }
  }
  ...
  private async executePhases(
    ...
    for (let pi = progress.currentPhase; pi < plan.phases.length; pi++) {
      ...
      this.emit({ type: 'build:phase_started', buildId: progress.id, phaseIndex: pi, timestamp: new Date().toISOString(), data: { message: `Phase ${pi + 1}/${plan.phases.length}: ${phase.name}`, progress: this.progressSummary(progress, phase.name) } })

      await this.executePhaseTasks(progress, phase, baseOptions, maxBudgetUsd)

      // Capture project state
      const snapshot = capturePhaseSnapshot(baseOptions.projectRoot, phase.name)
      ...
      if (phase.verify && !this.cancelled) {
        ...
        const result = runVerification(baseOptions.projectRoot, baseOptions.nodePath)

        this.emit({ type: 'build:phase_verified', buildId: progress.id, phaseIndex: pi, timestamp: new Date().toISOString(), data: { message: result.passed ? `Phase ${pi + 1} verification passed` : `Phase ${pi + 1} verification issues: ${result.summary}` } })

        if (!result.passed) {
          const decision = await this.humanInput.handleVerificationFailure(
            progress.id, pi, result.output,
          )

          if (decision === 'abort') {
            this.markRemainingSkipped(progress, 'Aborted after verification failure')
            break
          }
      ...
      this.emit({ type: 'build:phase_completed', buildId: progress.id, phaseIndex: pi, timestamp: new Date().toISOString(), data: { message: `Phase ${pi + 1} complete: ${phase.name}`, progress: this.progressSummary(progress, phase.name), costUsd: progress.totalCostUsd } })
      ...
      if (pi < plan.phases.length - 1 && !this.cancelled) {
        ...
        const continueToNext = await this.humanInput.approvePhase(progress.id, pi, phaseSummary)

        if (!continueToNext) {
          this.markRemainingSkipped(progress, 'Stopped by user after phase review')
          break
        }
      }
    }
  ...
  private async executePhaseTasks(
    ...
    for (const task of phase.tasks) {
      ...
      if (r && (r.status === 'completed' || r.status === 'skipped')) completed.add(task.id)
    }
    ...
    while (completed.size < phase.tasks.length) {
      ...
      if (ready.length === 0) {
        for (const t of phase.tasks) {
          if (!completed.has(t.id)) {
            ...
            if (r.status === 'pending') { r.status = 'skipped'; r.error = 'Dependency not met' }
            completed.add(t.id)
          }
        }
        break
      ...
      await Promise.allSettled(ready.map((task) => this.executeTask(progress, task, baseOptions)))
  ...
  private async executeTask(
    ...
    this.emit({ type: 'build:task_started', buildId: progress.id, taskId: task.id, timestamp: new Date().toISOString(), data: { message: `[${task.role}] ${task.title}`, role: task.role, progress: this.progressSummary(progress, task.title) } })

    // Accumulated context
    const contextParts: string[] = []
    if (progress.buildContext.length > 0) {
      contextParts.push('## What Has Been Built So Far\n' + progress.buildContext[progress.buildContext.length - 1])
    }
    if (task.dependsOn.length > 0) {
      const depSummaries = task.dependsOn
        .map((depId) => { const dep = progress.results.get(depId); return dep?.summary ? `- ${depId}: ${dep.summary}` : null })
        .filter(Boolean)
      if (depSummaries.length > 0) contextParts.push('## Prerequisite Task Results\n' + depSummaries.join('\n'))
    }
    ...
    try {
      ...
      if (execution.status === 'done') {
        ...
        this.emit({ type: 'build:task_completed', buildId: progress.id, taskId: task.id, timestamp: new Date().toISOString(), data: { message: `[${task.role}] ${task.title} — done`, role: task.role, costUsd: execution.costUsd, durationMs: execution.durationMs, progress: this.progressSummary(progress, task.title) } })
      } else {
        await this.handleTaskFailure(progress, task, baseOptions, execution.error ?? 'Unknown error', attempt)
      }
    } catch (err: any) {
      await this.handleTaskFailure(progress, task, baseOptions, err.message, attempt)
    }
  }
  ...
  private async handleTaskFailure(
    ...
    if (attempt < MAX_TASK_RETRIES) {
      progress.retryCount++
      this.emit({ type: 'build:task_retrying', buildId: progress.id, taskId: task.id, timestamp: new Date().toISOString(), data: { message: `[${task.role}] ${task.title} failed, retrying (${attempt + 1}/${MAX_TASK_RETRIES}): ${error}`, role: task.role } })
    ...
    this.emit({ type: 'build:task_failed', buildId: progress.id, taskId: task.id, timestamp: new Date().toISOString(), data: { message: `[${task.role}] ${task.title} failed after ${MAX_TASK_RETRIES + 1} attempts: ${error}`, role: task.role } })

    const decision = await this.humanInput.handleFailure(
      progress.id, task.id, task.title, error,
    )
    ...
    switch (decision) {
      ...
      case 'abort':
        ...
        result.error = error
        this.markRemainingSkipped(progress, 'Aborted by user after task failure')
        this.cancelled = true
        break
    }
  }
  ...
  private formatPhaseResult(progress: BuildProgress, phase: BuildPhase, phaseIndex: number): string {
    const lines: string[] = [
    ...
    for (const task of phase.tasks) {
      ...
      if (!r) continue
      const icon = r.status === 'completed' ? 'OK' : r.status === 'skipped' ? 'SKIPPED' : 'FAILED'
  ...
  private initProgress(buildId: string, plan: BuildPlan, startedAt: string): BuildProgress {
    const results = new Map<string, BuildTaskResult>()
    for (const phase of plan.phases) {
      for (const task of phase.tasks) {
        results.set(task.id, { taskId: task.id, role: task.role, status: 'pending', executionId: null, summary: null, error: null, costUsd: 0, durationMs: 0, sessionId: null })
      }
    }
    return { id: buildId, plan, status: 'executing', currentPhase: 0, results, buildContext: [], totalCostUsd: 0, totalDurationMs: 0, startedAt, completedAt: null, retryCount: 0 }
  }

  private markRemainingSkipped(progress: BuildProgress, reason: string): void {
    for (const [, result] of progress.results) {
      if (result.status === 'pending') { result.status = 'skipped'; result.error = reason }
    }
  }

  private progressSummary(progress: BuildProgress, currentLabel: string): { completed: number; total: number; phase: string } {
    let completed = 0
    for (const [, r] of progress.results) { if (r.status === 'completed') completed++ }
    return { completed, total: progress.plan.totalTasks, phase: currentLabel }
  }

  private emit(event: BuildEvent): void {
    for (const cb of this.listeners) {
      try { cb(event) } catch (err) { console.error('[project-builder] Event listener error:', err) }
    }
  }
}
