...
export class BackendEngineerAgent extends DecomposableAgent {
  ...
  protected transformPrompt(prompt: string): string {
    return [
      ...
      "- FIRST: Read the project to identify the backend framework and match its conventions exactly.",
      "- Modularize: one model/entity per file, one view/controller group per file. If a module has multiple definitions, split into a folder with a barrel re-export (__init__.py / index.ts).",
      "- Service layer for business logic. Controllers/views handle HTTP only.",
      "- Include proper serializers/schemas with validation for all API endpoints.",
