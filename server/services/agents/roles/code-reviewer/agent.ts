...
export class CodeReviewerAgent extends BaseAgent {
  ...
  protected buildExecutionContext(options: AgentExecuteOptions): string {
    ...
    try {
      ...
      const recentCommit = execSync('git log -1 --oneline --no-color 2>/dev/null', {
        cwd: options.projectRoot,
        encoding: 'utf-8',
        timeout: 5000,
      }).trim()
      ...
      if (uncommitted) parts.push(`### Unstaged Changes\n\`\`\`\n${uncommitted}\n\`\`\``)
  ...
  protected processResult(
    ...
    if (fullResponse.toLowerCase().startsWith('approved:')) return 'Approved: ' + fullResponse.slice(9).trim().slice(0, 100)
    if (fullResponse.toLowerCase().startsWith('changes needed:')) return 'Changes needed: ' + fullResponse.slice(15).trim().slice(0, 100)
