...
type DetectedRunner = Partial<RunnerConfigInput> & { name: string; command: string }
...
export function detectRunners(projectRoot: string): DetectedRunner[] {
  ...
  const readJson = (rel: string) => {
    try { return JSON.parse(fs.readFileSync(path.join(projectRoot, rel), 'utf-8')) }
    catch { return null }
  }
  ...
  // ── Django (standalone) ──
  if (exists('manage.py')) {
    const venvPython = exists('venv/bin/python') ? 'venv/bin/python' : exists('.venv/bin/python') ? '.venv/bin/python' : 'python'
    runners.push({
      name: 'Django',
