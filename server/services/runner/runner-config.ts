...
export type RunnerConfigInput = Omit<
  RunnerConfig,
...
export class RunnerConfigService {
  ...
  addConfig(
    projectId: string,
    input: Partial<RunnerConfigInput> & { name: string; command: string },
  ): RunnerConfig {
    const id = crypto.randomUUID();
    const now = new Date().toISOString();

    this.db
      .insert(runnerConfigs)
      .values({
        id,
        projectId,
        name: input.name,
  ...
  updateConfig(
    ...
    if (partial.readyPattern !== undefined)
      updates.readyPattern = partial.readyPattern;
    if (partial.previewUrl !== undefined)
    ...
    if (partial.autoDetected !== undefined)
      updates.autoDetected = partial.autoDetected;

    if (Object.keys(updates).length > 0) {
      this.db
        .update(runnerConfigs)
        .set(updates)
        .where(eq(runnerConfigs.id, id))
        .run();
    }
  ...
  reorderConfigs(projectId: string, orderedIds: string[]): void {
    for (let i = 0; i < orderedIds.length; i++) {
      this.db
        ...
        .where(
          and(
            eq(runnerConfigs.id, orderedIds[i]),
            eq(runnerConfigs.projectId, projectId),
          ),
        )
        .run();
    }
