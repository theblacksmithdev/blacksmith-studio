...
export const claude = {
  ...
  onToolUse: (cb: (data: ClaudeToolUseEvent) => void) => raw.subscribe('claude:onToolUse', cb),
  onDone: (cb: (data: ClaudeDoneEvent) => void) => raw.subscribe('claude:onDone', cb),
  onError: (cb: (data: ClaudeErrorEvent) => void) => raw.subscribe('claude:onError', cb),
} as const
