...
export const TreeNode = memo(function TreeNode({
  ...
  return (
    <>
      ...
      >
        {/* Chevron */}
        <Box css={{ width: '14px', flexShrink: 0, display: 'flex', justifyContent: 'center', color: 'var(--studio-text-muted)' }}>
          {isDir && (open ? <ChevronDown size={11} /> : <ChevronRight size={11} />)}
        </Box>

        {/* File label */}
        <Box css={{ flex: 1, minWidth: 0 }}>
          <FileLabel name={item.name} isDir={isDir} isOpen={open} isSelected={isSelected} />
        </Box>
