...
export function OpenInEditorButton() {
  const { editors, preferred, setPreferred, openFile, isLoading } = useEditors()
    ...
    <>
      <ButtonWrap ref={btnRef}>
        <Tooltip content={label}>
          <IconButton variant="ghost" size="sm" onClick={handleClick} aria-label={label}>
            <ExternalLink />
          </IconButton>
        </Tooltip>
      </ButtonWrap>
        ...
        <>
          ...
          <Dropdown style={{ top: pos.top, left: pos.left }}>
            <div style={{ padding: '6px 10px 4px' }}>
              <Text variant="caption" color="muted">Open project in...</Text>
            </div>
            {editors.map((editor) => (
              <EditorRow
                key={editor.id}
              ...
              >
                ...
                {editor.command === preferred?.command && (
                  <CheckMark><Check size={13} /></CheckMark>
                )}
              </EditorRow>
            ))}
          </Dropdown>
