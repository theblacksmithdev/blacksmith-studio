...
function Field({ label, value, mono }: { label: string; value?: string | null; mono?: boolean }) {
  ...
  return (
    <Flex direction="column" gap={spacing.xs}>
      ...
      <Text variant="bodySmall" css={mono ? { fontFamily: "'SF Mono', monospace", fontSize: '12px' } : {}}>
        {value}
      </Text>
    </Flex>
  )
...
export function ConfigDetail({ config }: ConfigDetailProps) {
  ...
  return (
    <Flex direction="column" gap={spacing.xl}>
          ...
          <Flex direction="column" gap={spacing.xs} css={{
            ...
            {envEntries.map(([key, value]) => (
              <Flex key={key} align="center" gap={spacing.xs}>
                <Text variant="caption" css={{ fontFamily: "'SF Mono', monospace", fontWeight: 500, color: 'var(--studio-text-primary)' }}>
                  {key}
                </Text>
                <Text variant="caption" color="muted">=</Text>
                <Text variant="caption" css={{ fontFamily: "'SF Mono', monospace", color: 'var(--studio-text-secondary)' }}>
                  {value}
                </Text>
              </Flex>
            ))}
      ...
      {config.autoDetected && (
        <Badge variant="default" size="sm" css={{ alignSelf: 'flex-start' }}>Auto-detected</Badge>
      )}
    </Flex>
  )
}
