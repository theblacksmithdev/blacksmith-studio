...
export function KnowledgeRow({ name, size, updatedAt, onEdit, onDelete }: KnowledgeRowProps) {
  const menuOptions: MenuOption[] = [
    { icon: <Pencil />, label: 'Edit', onClick: onEdit },
    { icon: <Trash2 />, label: 'Delete', onClick: onDelete, danger: true, separator: true },
  ]
  ...
  return (
    ...
    >
      <FileText size={14} style={{ color: 'var(--studio-text-muted)', flexShrink: 0 }} />
      ...
      <Box css={{ flex: 1, minWidth: 0 }}>
        ...
        <Text css={{ fontSize: '11px', color: 'var(--studio-text-muted)', marginTop: '1px' }}>
          {(size / 1024).toFixed(1)} KB · {formatDistanceToNow(new Date(updatedAt), { addSuffix: true })}
        </Text>
      </Box>
