...
export function ConversationsList() {
  ...
  const goNew = () => {
    if (pid) navigate(agentsNewPath(pid));
  };
  const goConv = (id: string) => {
    if (pid) navigate(agentsConversationPath(pid, id));
  };
      ...
      <Content>
        <Stack>
          <HeroWrap>
            ...
            <HeroSub>
              Your AI engineering team — PM, engineers, designers, QA —
              coordinated to build features together.
            </HeroSub>
          </HeroWrap>

          <RosterWrap>
            {roster.map(({ icon: Icon, label }) => (
              <RosterChip key={label}>
                <Icon size={10} />
                {label}
              </RosterChip>
            ))}
              ...
              <VStack gap="4px" align="stretch" w="100%">
                {convs.map((c) => (
                  ...
                  >
                    ...
                    <DeleteBtn onClick={(e) => handleDelete(e, c.id)}>
                      <Trash2 size={13} />
                    </DeleteBtn>
                  </ConvCard>
                ))}
