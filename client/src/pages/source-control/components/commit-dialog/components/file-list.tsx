...
import { Text, Badge, Button, InfiniteScrollList, SkeletonList, spacing, radii } from '@/components/shared/ui'
...
export function FileList({ files, total, selectedCount, isAllSelected, isFileSelected, onToggle, onToggleAll, onLoadMore, isLoadingMore }: FileListProps) {
  const renderItem = useCallback((file: GitChangedFile) => (
    <FileRow file={file} isSelected={isFileSelected(file.path)} onToggle={onToggle} />
    ...
    <Flex direction="column" gap={spacing.sm}>
      <Flex align="center" justify="space-between">
        <Flex align="center" gap={spacing.xs}>
          ...
          <Badge variant="default" size="sm">{selectedCount}/{total}</Badge>
        </Flex>
      ...
      <Box css={{ height: '240px', borderRadius: radii.md, border: '1px solid var(--studio-border)', overflow: 'hidden' }}>
        <InfiniteScrollList
          items={files}
          estimateSize={32}
