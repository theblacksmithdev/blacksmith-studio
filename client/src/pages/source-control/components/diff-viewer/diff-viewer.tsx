...
function Shell({ children }: { children: React.ReactNode }) {
  return (
    <Flex direction="column" css={{ height: '100%', background: 'var(--studio-bg-main)', overflow: 'hidden' }}>
      {children}
    </Flex>
  )
}
...
export function DiffViewer({ filePath, onClose }: DiffViewerProps) {
  ...
  if (!filePath) {
    return <Shell><NoFileSelected /></Shell>
  }
  ...
  if (!parsed || parsed.lines.length === 0) {
    return <Shell><NoChanges filePath={filePath} onClose={onClose} /></Shell>
  }
