...
export const useFileStore = create<FileState>((set, get) => ({
  ...
  openFile: (path) => {
    ...
    if (!exists) {
      set({
        openTabs: [
          ...openTabs,
          { path, content: null, originalContent: null, language: "text" },
        ],
        activeTab: path,
      });
    } else {
    ...
    set((s) => {
      ...
      return {
        openTabs: s.openTabs.map((t) =>
          t.path === path
            ? { ...t, content, originalContent: content, language, error: null }
            : t,
        ),
        changedFiles: next,
      };
  ...
  setTabError: (path, error) => {
    set((s) => ({
      openTabs: s.openTabs.map((t) => (t.path === path ? { ...t, error } : t)),
    }));
  },
