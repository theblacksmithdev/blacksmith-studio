...
const FIELDS: FieldDef[] = [
  { name: "name", label: "Name", placeholder: "Frontend" },
  {
    ...
    hint: "Use {port} as a placeholder for the resolved port",
    mono: true,
  },
  {
    name: "setupCommand",
    ...
    hint: "Runs before starting the service (e.g. install dependencies)",
    mono: true,
  },
  {
    name: "cwd",
  ...
  { name: "port", label: "Default port", placeholder: "3000" },
  {
    name: "readyPattern",
    label: "Ready pattern",
    placeholder: "Local:|ready|listening",
    hint: "Regex matched against stdout to detect running state",
    mono: true,
  },
  {
    name: "previewUrl",
    ...
    hint: "{port} is replaced at runtime",
    mono: true,
  },
];
      ...
      {FIELDS.map((field) => (
        <Box key={field.name}>
          ...
          {errors[field.name as keyof typeof errors] && (
            <Text
              variant="tiny"
              css={{ color: "var(--studio-error)", marginTop: spacing.xs }}
            >
