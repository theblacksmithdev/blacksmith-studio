...
export function MessageList({
  messages, maxWidth = '760px', emptyState, trailing, renderMessage, renderContent,
}: MessageListProps) {
  ...
  if (messages.length === 0 && !trailing) {
    return (
      <Flex css={{ flex: 1 }}>
        {emptyState ?? <EmptyState compact icon={<MessageSquare />} description="Send a message to start the conversation" />}
      </Flex>
    )
  }
    ...
    <Box flex={1} overflowY="auto">
      <Box css={{
        ...
        {messages.map((msg, i) => {
          ...
          const showSeparator = i > 0 && msg.role === 'user' && prevMsg?.role !== 'user'

          return (
            <Box key={msg.id}>
              {showSeparator && (
                <Box css={{
                  height: '1px', background: 'var(--studio-border)',
                  margin: `${spacing.xs} 0 ${spacing.xl}`, opacity: 0.5,
                }} />
              )}
