...
function CopyButton({ content }: { content: string }) {
  ...
  return (
    <Tooltip content={copied ? 'Copied' : 'Copy'}>
      <IconButton variant="ghost" size="xs" onClick={handleCopy} aria-label="Copy">
        {copied ? <Check /> : <Copy />}
      </IconButton>
    </Tooltip>
  )
...
export const MessageBubble = memo(function MessageBubble({ message, renderContent }: MessageBubbleProps) {
    ...
    return (
      <Flex direction="column" align="flex-end" gap={spacing.xs} css={{
        ...
        <Box css={{
          ...
          <Flex className="msg-actions" align="center" gap={spacing.xs} css={{
            position: 'absolute', bottom: `-${spacing.xl}`, right: 0,
    ...
    <Flex direction="column" gap={spacing.xs} css={{
      ...
      {(message.senderIcon || message.senderName) && (
        <Flex align="center" gap={spacing.sm}>
          {message.senderIcon}
          {message.senderName && (
            <Text variant="label" css={{ fontWeight: 600, color: 'var(--studio-text-secondary)' }}>
              {message.senderName}
            </Text>
          )}
          {time && <Text variant="tiny" color="muted">{time}</Text>}
      ...
      <Box css={{ paddingLeft: message.senderIcon ? '30px' : 0 }}>
        {renderContent ? renderContent(message.content) : (
          <Text variant="body" css={{ lineHeight: 1.6, whiteSpace: 'pre-wrap', color: 'var(--studio-text-primary)' }}>
            {message.content}
          </Text>
        )}
        {message.metadata}
