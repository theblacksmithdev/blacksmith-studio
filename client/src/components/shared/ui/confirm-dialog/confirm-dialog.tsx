...
export function ConfirmDialog({
  ...
  return createPortal(
    ...
    >
      ...
      >
        <Box css={{ padding: `${spacing['2xl']} ${spacing['2xl']} ${spacing.lg}` }}>
          <Text variant="subtitle" css={{ marginBottom: spacing.xs }}>{message}</Text>
          {description && (
            <Text variant="body" color="muted" css={{ lineHeight: 1.55 }}>{description}</Text>
          )}
        </Box>
