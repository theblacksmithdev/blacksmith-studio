...
const TYPE_INFO: Record<ProjectType, { icon: typeof Layers; label: string; tag: string; description: string }> = {
  empty: { icon: FolderPlus, label: 'Empty Project', tag: 'Any stack', description: 'Blank folder — bring your own tools and framework' },
  fullstack: { icon: Layers, label: 'Full Stack', tag: 'Django + React', description: 'Backend API + Frontend UI with OpenAPI type sync' },
  backend: { icon: Server, label: 'Backend', tag: 'Django', description: 'Standalone REST API with DRF and JWT auth' },
  frontend: { icon: Monitor, label: 'Frontend', tag: 'React', description: 'Standalone React app with Vite and Chakra UI' },
}
...
export function CreateStep({ onClose }: { onClose: () => void }) {
  ...
  const { register, handleSubmit, setValue, watch, formState: { errors } } = useForm<z.infer<typeof createSchema>>({
    ...
    defaultValues: { name: '', parentPath: '', type: 'fullstack', backendPort: 8000, frontendPort: 5173, theme: 'default' },
  })
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <SplitLayout>
        {/* ── Left: Type selector ── */}
        <LeftPanel>
          <Text variant="tiny" color="muted" css={{ marginBottom: spacing.sm }}>Project type</Text>
          {PROJECT_TYPES.map((t) => {
            const info = TYPE_INFO[t]
            const Icon = info.icon
            const active = projectType === t
            return (
              <TypeOption key={t} type="button" $active={active} onClick={() => setValue('type', t)}>
                <TypeIconWrap $active={active}>
                  <Icon size={14} />
                </TypeIconWrap>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <Text variant="label" css={{ color: active ? 'var(--studio-text-primary)' : 'var(--studio-text-secondary)', fontWeight: active ? 500 : 400, display: 'block' }}>
                    {info.label}
                  </Text>
                  <Text variant="caption" color="muted">{info.tag}</Text>
                </div>
              </TypeOption>
            )
          })}
        ...
        <RightPanel>
          ...
          <TypeDetail>
            <HStack gap="sm" css={{ marginBottom: spacing.xs }}>
              <CurrentIcon size={14} style={{ color: 'var(--studio-text-secondary)' }} />
              <Text variant="label" css={{ color: 'var(--studio-text-primary)', fontWeight: 500 }}>{currentInfo.label}</Text>
              <Badge variant="outline" size="sm">{currentInfo.tag}</Badge>
            </HStack>
            <Text variant="caption" color="muted">{currentInfo.description}</Text>
          </TypeDetail>
          ...
          <VStack gap="md">
            ...
            <Button variant="primary" size="lg" css={{ width: '100%', marginTop: spacing.sm }} onClick={handleSubmit(onSubmit)}>
