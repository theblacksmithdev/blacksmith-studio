...
import { Text, Input, Button, IconButton, Tooltip, spacing, radii } from '@/components/shared/ui'
...
export function EnvEditor({ register, envArray }: EnvEditorProps) {
  ...
  const switchToBulk = () => {
    setBulkText(fields.map((f) => `${f.key}=${f.value}`).filter((l) => l !== '=').join('\n'))
    setBulkMode(true)
  }
        ...
        <Flex gap={spacing.xs}>
          ...
          {!bulkMode && (
            <Tooltip content="Add variable">
              <IconButton variant="ghost" size="xs" onClick={() => append({ key: '', value: '' })} aria-label="Add variable">
                <Plus />
              </IconButton>
            </Tooltip>
          )}
      ...
      {bulkMode ? (
        ...
        <Flex direction="column" gap={spacing.sm}>
          <textarea
            ...
            placeholder={'# Paste environment variables\nKEY=value\nDATABASE_URL=postgres://...\nNODE_ENV=development'}
            rows={8}
            style={{
              width: '100%',
              padding: spacing.sm,
          ...
          <Flex gap={spacing.sm} justify="flex-end">
            ...
            <Button variant="primary" size="sm" onClick={applyBulk}>Apply</Button>
          </Flex>
        </Flex>
      ) : fields.length === 0 ? (
        /* ── Empty state ── */
        <Flex direction="column" align="center" gap={spacing.sm} css={{ padding: spacing.xl }}>
          <Text variant="caption" color="muted">No environment variables</Text>
          <Flex gap={spacing.sm}>
            <Flex
              as="button"
              align="center"
          ...
          {fields.map((field, i) => (
            <Flex key={field.id} align="center" gap={spacing.xs}>
              <Input
                size="sm"
                placeholder="KEY"
                css={{ flex: 1, fontFamily: "'SF Mono', monospace", fontSize: '12px' }}
                {...register(`env.${i}.key`)}
              />
              ...
              <Input
                size="sm"
                placeholder="value"
                css={{ flex: 2, fontFamily: "'SF Mono', monospace", fontSize: '12px' }}
                {...register(`env.${i}.value`)}
              />
              <Tooltip content="Remove">
                <IconButton variant="ghost" size="xs" onClick={() => remove(i)} aria-label="Remove">
                  <Trash2 />
                </IconButton>
              </Tooltip>
            </Flex>
