...
const Card = styled.div<{ active: boolean; variant: CardVariant }>`
  ...
  border: ${({ variant }) =>
    variant === "inline"
      ? "none"
      : `1px solid ${({ active }: any) => (active ? "var(--studio-border-hover)" : "var(--studio-border)")}`};
...
const ActionBtn = styled.button<{ size: number; danger?: boolean }>`
  ...
  background: var(--studio-bg-sidebar);
  color: ${({ danger }) =>
    danger ? "var(--studio-text-secondary)" : "var(--studio-accent)"};
  display: flex;
  align-items: center;
  justify-content: center;
  cursor: pointer;
    ...
    <Card active={active} variant={variant}>
      ...
      <Actions>
        ...
        <ActionBtn
          ...
          danger={active}
          onClick={onToggle}
          title={active ? `Stop ${label}` : `Start ${label}`}
        >
