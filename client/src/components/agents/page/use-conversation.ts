...
export function useConversation(conversationId: string | undefined) {
  ...
  useEffect(() => {
    ...
    if (currentConvId && projectId) {
      api.agents.listChat(projectId, currentConvId).then((messages) => {
  ...
  const handleSend = useCallback(async (message: string) => {
    ...
    if (!convId) {
      try {
        const conv = await api.agents.createConversation(projectId!, message.slice(0, 60))
        convId = conv.id as string
        setCurrentConvId(convId)
        if (projectId && convId) {
          navigate(agentsConversationPath(projectId, convId), { replace: true })
        }
      } catch (err: any) {
