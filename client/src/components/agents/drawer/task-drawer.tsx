              ...
              <SummaryCard>
                ...
                <Flex gap={2} mt="8px" align="center">
                  ...
                  {errored > 0 && (
                    <>
                      <Separator>·</Separator>
                      <StatChip $color="var(--studio-error)">
                        {errored} failed
                      </StatChip>
                    </>
                  )}
                  {skipped > 0 && (
                    <>
                      <Separator>·</Separator>
                      <StatChip $color="var(--studio-text-muted)">
                        {skipped} skipped
                      </StatChip>
                    </>
                  )}
                      ...
                      <Box css={{ flex: 1, minWidth: 0 }}>
                        <Flex align="center" gap="6px">
                          <Text
                            css={{
                              ...
                              textDecoration:
                                status === "skipped" ? "line-through" : "none",
                              flex: 1,
                              minWidth: 0,
                              whiteSpace: "nowrap",
                              overflow: "hidden",
                          ...
                          <ModelTag $model={task.model ?? "balanced"}>
                            {modelLabel(task.model)}
                          </ModelTag>
                          <RoleTag $active={isActive}>
                            {roleLabel(task.role)}
                          </RoleTag>
                        </Flex>
                        {task.description && (
                        ...
                        >
                          ...
                          {agentSubtasks.length > 0 &&
                            ` · ${agentSubtasks.filter((s) => s.status === "done").length}/${agentSubtasks.length} sub-tasks`}
                          {(task as any).error && status === "error" && (
                            <span style={{ color: "var(--studio-error)" }}>
                              {" "}
                              — {String((task as any).error).slice(0, 80)}
                            </span>
                          )}
                        </Text>
                      </Box>
