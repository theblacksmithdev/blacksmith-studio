...
export function setupFilesIPC(projectManager: ProjectManager) {
  ...
  ipcMain.handle(FILES_RENAME, (_e, data: { path: string; newName: string }) => {
    ...
    if (!oldFull.startsWith(projectPath)) throw new Error('Access denied: path outside project root')
    const newFull = path.resolve(path.dirname(oldFull), data.newName)
    if (!newFull.startsWith(projectPath)) throw new Error('Access denied: path outside project root')
  ...
  ipcMain.handle(FILES_DELETE, (_e, data: { path: string }) => {
    ...
    if (!projectPath) throw new Error('No active project.')
    const fullPath = path.resolve(projectPath, data.path)
    if (!fullPath.startsWith(projectPath)) throw new Error('Access denied: path outside project root')
