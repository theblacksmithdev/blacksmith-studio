...
export function setupFilesIPC(projectManager: ProjectManager) {
  ...
  ipcMain.handle(FILES_CONTENT, (_e, data: { projectId: string; path: string }) => {
    const projectPath = resolveProjectPath(projectManager, data.projectId)
    if (!data.path) throw new Error('path is required')
    return readFileContent(projectPath, data.path)
  })

  ipcMain.handle(FILES_SEARCH, (_e, data: { projectId: string; query: string; maxResults?: number }) => {
    const projectPath = resolveProjectPath(projectManager, data.projectId)
    if (!data.query) return []
    return searchFileContents(projectPath, data.query, data.maxResults)
  })

  ipcMain.handle(FILES_REVEAL, (_e, data: { projectId: string; path: string }) => {
    const projectPath = resolveProjectPath(projectManager, data.projectId)
  ...
  ipcMain.handle(FILES_OPEN_IN_EDITOR, (_e, data: { projectId: string; path: string; command?: string }) => {
    const projectPath = resolveProjectPath(projectManager, data.projectId)
    const fullPath = path.resolve(projectPath, data.path)
    openInEditor(data.command || 'code', fullPath)
  })

  ipcMain.handle(FILES_SAVE, (_e, data: { projectId: string; path: string; content: string }) => {
    const projectPath = resolveProjectPath(projectManager, data.projectId)
    if (!data.path) throw new Error('path is required')
    writeFileContent(projectPath, data.path, data.content)
  })

  ipcMain.handle(FILES_RENAME, (_e, data: { projectId: string; path: string; newName: string }) => {
    const projectPath = resolveProjectPath(projectManager, data.projectId)
  ...
  ipcMain.handle(FILES_DELETE, (_e, data: { projectId: string; path: string }) => {
    const projectPath = resolveProjectPath(projectManager, data.projectId)
    const fullPath = path.resolve(projectPath, data.path)
