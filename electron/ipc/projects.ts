...
export function setupProjectsIPC(
  ...
  ipcMain.handle(
    PROJECTS_REGISTER,
    (_e, data: { path: string; name?: string }) => {
  ...
  ipcMain.handle(
    PROJECTS_REMOVE,
    async (_e, data: { id: string; hard?: boolean }) => {
      const project = projectManager.get(data.id);
      if (!project) throw new Error("Project not found");
  ...
  ipcMain.handle(
    ...
    (
      ...
      if (!data.name || !data.parentPath)
        throw new Error("name and parentPath are required");
      ...
      if (fs.existsSync(projectDir)) {
        throw new Error(
          `Directory "${data.name}" already exists in ${absParent}`,
        );
      }
      ...
      const args = [
        ...
        data.type || "fullstack",
        "-b",
        String(data.backendPort || 8000),
        "-f",
        String(data.frontendPort || 5173),
        "-t",
        data.theme || "default",
      ];
      ...
      const win = getWindow();
      const nodePath =
        settingsManager.resolve(data.projectId ?? null, "runner.nodePath") ||
        undefined;
      ...
      const proc = spawn(
        nodeCmd("npx", nodePath),
        ["blacksmith-cli@0.1.10", ...args],
        {
          cwd: absParent,
  ...
  ipcMain.handle(
    ...
    (
      ...
      if (!data.gitUrl || !data.parentPath)
        throw new Error("gitUrl and parentPath are required");
      ...
      if (fs.existsSync(projectDir)) {
        throw new Error(
          `Directory "${repoName}" already exists in ${absParent}`,
        );
      }
      ...
      const proc = spawn(
        "git",
        ["clone", "--progress", data.gitUrl, repoName],
        {
          cwd: absParent,
          stdio: ["pipe", "pipe", "pipe"],
  ...
  ipcMain.handle(BROWSE_LIST, (_e, data?: { path?: string }) => {
    ...
    return {
      ...
      dirs,
      isProject:
        fs.existsSync(path.join(absPath, "blacksmith.config.json")) ||
        fs.existsSync(path.join(absPath, "package.json")) ||
        fs.existsSync(path.join(absPath, ".git")),
      isBlacksmithProject: fs.existsSync(
        path.join(absPath, "blacksmith.config.json"),
      ),
    };
  });
}
