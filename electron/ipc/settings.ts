...
export function setupSettingsIPC(settingsManager: SettingsManager, projectManager: ProjectManager) {
  ...
  ipcMain.handle(SETTINGS_UPDATE, (_e, data: { projectId: string; settings: Record<string, any> }) => {
    const id = resolveProject(projectManager, data.projectId)
    if (!data.settings || typeof data.settings !== 'object') throw new Error('settings must be a JSON object')
